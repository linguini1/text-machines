...
Characters are processed one-by-one, which can be slow. Although characters are buffered when read from a file to avoid
extra system calls and instead operate mostly directly on memory, chained text machines are forced to operate on a
character-by-character basis and perform their own buffering if they need memory.
